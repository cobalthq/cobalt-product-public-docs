...
Serialization is the process of converting an object to a byte stream, which can then be sent over a network or saved on a disk. The most common example would be the `pickle.dumps()` function in Python3 which serializes any input data. 
...
- However, even these libraries can be susceptible to vulnerabilities. For example, CVE-2020-1747, CVE-2020-14343, etc.
...
- Tampering of Application Logic
...
The resulting output would be a payload, which when sent in the POST body would execute the respective command on the system.
...
-   In the case of Java applications, if you do need to implement the `serializable` interface due to inheritance, you can override the `readObject()` to prevent actual deserialization. Take the following example from [Snyk](https://snyk.io/):
...
```
- In the case of PHP applications, ue a safe, standard data interchange format such as `JSON` (via `json_decode()` and `json_encode()`) instead of `unserialize()` function, if you need to pass serialized data to the user.
